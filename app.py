...
@app.route("/")
def index():
    return render_template("index.html", message="Last session:", sent_message=session["text_message"], **session["statuses"], image_url=g.image_url, numbers=utils.get_display_numbers(g.data))
...
def upload():
    ...
    ext = utils.allowed_file(upload_file.filename)
